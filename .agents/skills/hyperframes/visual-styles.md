...
| Soft Warm       | Gentle, clear     | Education, tutorials, explainers   | Simple crossfade                  |
...
## 9. Soft Warm — Knowledge & Education

**Mood:** Gentle, clear | **Best for:** Knowledge videos, tutorials, explainers, education content

- Warm white background (`#F5F0EB`) — same across ALL scenes, never changes
- Primary: `#3D5A80` (lines, shapes, strokes). Accent: `#E07A5F` (highlights, key terms). Secondary: `#81B29A` (supporting elements). Text: `#2B2D42`
- Noto Sans SC (Chinese) / Inter (Latin). Bold 700+ for headlines 40-48px, Regular for body
- All shapes use rounded corners (rx >= 12). Stroke-linecap: round. Stroke width 2-3px
- Concrete icons over abstract shapes — draw a computer, not a rectangle
- Background has subtle texture (dot grid or fine lines at 5-8% opacity) plus scattered decorative elements (small dots, crosses, short dashes) in #3D5A80 at low opacity
- Minimum 8 visible objects per scene — no large empty areas
- Transitions: simple crossfade between scenes. No shader transitions.

**GSAP signature:** `back.out(1.4)` for entrances (bouncy overshoot), `power2.inOut` for motion paths. Elements feel physical, not mechanical.

**Prohibited:**
- `<rect>` with no rx as a stand-in for real objects
- `system-ui` or any system font
- Pure gray fills (#334155, #475569, etc.)
- Gradient backgrounds or gradient text
- Dark/black backgrounds (#0f172a, #1e293b, #000)

```
Soft Warm: Warm white #F5F0EB background, never changes between scenes.
Rounded shapes, concrete icons, scattered decoratives. Noto Sans SC bold headlines.
Bouncy entrances, physical motion. Simple crossfades. Minimum 8 objects per scene.
```
